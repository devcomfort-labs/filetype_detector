...
| `.macho` | 64-bit Mach-O header and load command; `.macho` is a non-standard extension and does not by itself establish MIME authority | `ExecutableGenerator._create_macho` | `macholib==1.16.4` parses the Mach-O header | `python -m pip install -r .audit/requirements-macho.txt`, or run the OS installer script | `scripts/generators/executables.py`; `.audit/macho_validate.py` |
